...
void Model::draw(Shader* shader, Material* draw_material, bool is_selected, bool disable_depth_test, bool render_only_ambient, bool render_one_color)
{
    for (unsigned int i = 0; i < meshes.size(); i++) {
        ...
        if (i == 0 || i == 11 || i == 15 || i == 19 || i == 22 || i == 29 || i == 30 || i == 60)
        {
            shader->setFloat("roughness_model", 0.2f);
            shader->setFloat("metalness_model", 0.5f);
            //shader->setVec3("emission_model", glm::vec3(0.0, 0.0, 0.0));
            shader->setVec3("emission_strength", glm::vec3(0.0, 0.0, 0.0));

            meshes[i].draw(shader, draw_material, is_selected, disable_depth_test, render_only_ambient, render_one_color);
        }
        else if (meshes[i].name.find("wheel") != std::string::npos)
        {
        ...
        {
            shader->setVec3("emission_strength", glm::vec3(255.0, 0.0, 0.0));
            meshes[i].draw(shader, draw_material, is_selected, disable_depth_test, render_only_ambient, render_one_color);
        }
        else {
