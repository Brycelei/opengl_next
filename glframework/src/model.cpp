...
namespace NESI_NEXT {
    Model::Model(std::string const& path, bool gamma) : gammaCorrection(gamma)
    {
        ...
        {
            this->name = path.substr(i + 1, j - i - 1);
        }
        ...
        NESI_INFO("{0} Model loaded in {1} seconds", this->name, std::to_string(elapsed_seconds.count()));
    }
    ...
    void Model::draw(Shader* shader, Material* draw_material, bool is_selected, bool disable_depth_test, bool render_only_ambient, bool render_one_color)
    {
     ...
     /*       if (meshes[i].name.find("wheel") != std::string::npos)
            {
                shader->setFloat("roughness_model", 1.0f);
                shader->setFloat("metalness_model", 0.01f);
 ...
 /*   bool Model::intersected_ray(const glm::vec3& orig, const glm::vec3& dir, float& t) {
    ...
    void Model::loadModel(std::string const& path) {
        ...
        {
            NESI_FATAL("ERROR::ASSIMP:: {0}", importer.GetErrorString());
            return;
        }
    ...
    Mesh Model::processMesh(aiMesh* mesh, const aiScene* scene)
    {
        ...
        if (loaded_materials.find(mesh->mMaterialIndex) == loaded_materials.end()) {
            ...
            // 8 diffuse maps       
            std::vector<Texture*> diffuseMaps = loadMaterialTextures(ai_material, aiTextureType_DIFFUSE, TexDiffuse, scene, material_name);
            // 9 ambient maps
            std::vector<Texture*> ambientMaps = loadMaterialTextures(ai_material, aiTextureType_AMBIENT, TexAmbient, scene, material_name);
            ...
            {
                material->textures[TexDiffuse] = diffuseMaps[0];
            }
            if (ambientMaps.size() != 0)
            {
                material->textures[TexAmbient] = ambientMaps[0];
            }
        ...
        for (int i = 0; i < (int)mesh->mNumBones; i++) {
            ...
            if (umap_bone_name_to_id.find(bone_name) == umap_bone_name_to_id.end()) {
                bone_id = (int)umap_bone_name_to_id.size();
            ...
            for (int j = 0; j < (int)mesh->mBones[i]->mNumWeights; j++) {
                ...
                if (!added_bone_weight) {
                    ...
                    NESI_ERROR("ERROR: NOT ENOUGH SPACE TO ADD BONE WEIGHT, INCREASE VARIABLE: MAX_BONE_INFLUENCE");
                    assert(0);
                }
            }
        }
    ...
    void Model::print_loaded_textures(const std::map<std::string, Texture*>& loaded_textures) {
        for (auto it = loaded_textures.begin(); it != loaded_textures.end(); it++) {
            ...
            for (auto it = texture->types.begin(); it != texture->types.end(); it++) {
                NESI_INFO("Texture loaded at path:{0}, with {1} channels, types: {2}", texture->path, texture->num_channels, Texture::get_long_name_of_texture_type(*it));
            }
        }
    }
    ...
    unsigned int EmbeddedTextureFromFile(const aiTexture* texture, int& num_channels)
    {
        ...
        if (data) {
            ...
            NESI_ERROR("Embedded texture failed to load");
            stbi_image_free(data);
        }
    ...
    unsigned int TextureFromFile(const char* path, const std::string& directory, int& num_channels, bool gamma)
    {
        ...
        if (filename.find(".exr") != std::string::npos)
        {
            ...
            GLuint texHeight = 0;
            int nrComponents = 0;
            float* gExrRGBA = nullptr;
            const char* err = nullptr; // or nullptr in C++11
            int ret = LoadEXR(&gExrRGBA, (int*)&texWidth, (int*)&texHeight, filename.c_str(), &err);
            if (ret != TINYEXR_SUCCESS) {
                if (err) {
                    fprintf(stderr, "ERR : %s\n", err);
                    NESI_ERROR("Read ERR file failed!: {0}", err);
                ...
                glGenTextures(1, &textureID);
                /*float* piexl = nullptr;
                piexl = readImageEXR(filename, (int*)&texWidth, (int*)&texHeight);*/
                glGenTextures(1, &textureID);
                glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
                glBindTexture(GL_TEXTURE_2D, textureID);
                GLenum format = GL_RGBA;
                glTexImage2D(GL_TEXTURE_2D, 0, format, texWidth, texHeight, 0, format, GL_FLOAT, gExrRGBA);
                glGenerateMipmap(GL_TEXTURE_2D);
                glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
                glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
                glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);
                ...
                free(gExrRGBA); // release memory of image data
                return textureID;
            }
        }
        int width, height;
        ...
        {
            NESI_ERROR("Texture failed to load at path: {0}", path);
            stbi_image_free(data);
        }
