...
enum TextureType {
    TexAlbedo, TexNormal, TexMetalness, TexRoughness, TexEmission, TexAmbientOcclusion, TexSpecular, TexDiffuse, TexAmbient, TexLast
};
...
public:
    ...
    static std::string get_short_name_of_texture_type(TextureType texture_type) {
        ...
        else if (texture_type == TexDiffuse)
        {
            return "diffuse";
        }
        else if (texture_type == TexAmbient)
        {
            return "ambient";
        }
...
public:
    ...
    void draw(Shader* shader, Material* draw_material, bool disable_depth_test)
    {
        ...
        const unsigned int OFFSET_TEXTURES = 0; // Accounting for PBR indirect light textures (irradiance, prefilter and BRDF maps)
